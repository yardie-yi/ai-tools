...
## 2026-08-04 通用化与配置驱动重构

- 保留 `references/` 原有 Markdown 文件集合及主路由职责。
- 将模块名称、平台、框架、构建、产物、部署、调试、Git、日志、注释和插件契约迁移到 `.evospec/module.config.yaml`。
- reference 改为渐进读取配置，不再保存项目命令和代码框架示例。
- 规则改为读取配置开关；补齐 R006 在规则索引中的登记。
- 推板 bat 改为通用包装器，由 Python 从配置渲染场景命令。
- 新增纯净可移植模板，当前项目版继续保留原项目输入输出资料。
